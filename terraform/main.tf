...
resource "aws_iam_policy" "ssm_policy" {
  name        = "ssp-cart-service-ssm-policy-${var.environment}"
  description = "Allows reading specific SSM parameters for the cart service"
    ...
    Statement = [
      {
        ...
        Effect   = "Allow",
        Resource = "arn:aws:ssm:${var.aws_region}:${data.aws_caller_identity.current.account_id}:parameter/${var.redis_host_param_name}"
      }
    ]
  })
}
...
module "ecs_service" {
  ...
  environment_variables = [
    { name = "REDIS_HOST_PARAM_NAME", value = var.redis_host_param_name },
